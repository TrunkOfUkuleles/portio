    ...
    return (
        <div className='project-container'>
            ...
            <div className={`project-inner chat-inner ${select==="gifsms"?"selected":""}`} >
                <div style={{"fontFamily": 'KoHo'}} className={`chat-title ${select==="gifsms"?"":"title-unselected"}`} onClick={()=>{setSelect('gifsms')}} >GIFSMS</div>
                {select==="gifsms" && <Chat/> }
            </div>
            <div className={`project-inner tracker-inner ${select==="tracker"?"selected":""}`}>
                <div style={{"fontFamily": 'Dune', "fontSize":"1rem"}} className={`chat-title ${select==="tracker"?"":"title-unselected"}`} onClick={()=>setSelect('tracker')}>Covid Tracker</div>
